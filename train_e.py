...
def load_and_preprocess_data(csv_path):
    ...
    X = np.array(features)
    # 只取第三列 (e值) 作为目标，并重塑为 (n, 1)
    y = df.iloc[valid_indices, 2].values.reshape(-1, 1)
...
def train_model(X, y, config):
    ...
    for epoch in range(config['epochs']):
        ...
        if not np.isnan(val_loss) and val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), 'best_model_e_temp.pth')
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load('best_model_e_temp.pth'))
...
def plot_results(train_losses, val_losses, y_test, predictions):
    ...
    axes[1].set_title('e Parameter: Predicted vs True')
    axes[1].grid(True)
    
    # 预测误差分布
    e_error = y_test - predictions
    axes[2].hist(e_error, alpha=0.7, label='e Error', bins=30)
    axes[2].set_xlabel('Prediction Error')
    axes[2].set_ylabel('Frequency')
    axes[2].set_title('Prediction Error Distribution for e')
    axes[2].legend()
    axes[2].grid(True)
    
    plt.tight_layout()
    plt.savefig('training_results_e.png', dpi=300, bbox_inches='tight')
    plt.show()

def save_report_to_md(metrics, config, total_samples, descriptor_info, file_path='training_report_e.md'):
    """将训练报告保存到Markdown文件"""
    report = f"""# 模型训练报告 (e值预测)
...
def main():
    ...
    try:
        ...
        # 保存为新模型
        model_save_path = 'mlp_model_e_onehot_optimized.pth'
        model_save_dict = {
        ...
        if os.path.exists('best_model_e_temp.pth'):
            os.remove('best_model_e_temp.pth')

        print("\n" + "="*60)
        print("训练完成！")
        print(f"模型已保存到 '{model_save_path}'")
        print(f"训练结果图已保存到 'training_results_e.png'")
